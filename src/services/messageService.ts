...
const ROLE_MAP: Record<string, Role[]> = {
  all: [Role.ADMIN, Role.BURSAR, Role.TEACHER, Role.PARENT, Role.STUDENT],
  admin: [Role.ADMIN],
  teacher: [Role.TEACHER],
  parent: [Role.PARENT],
  student: [Role.STUDENT],
  staff: [Role.ADMIN, Role.BURSAR, Role.TEACHER],
};
...
export const messageService = {
  ...
  sendBroadcast: async (
    ...
    const validRecipients = recipients.filter(r =>
      payload.type === 'email' ? r.email : r.phone
    );
    ...
    const sendPromises = validRecipients.map(async (recipient) => {
      try {
        if (payload.type === 'email') {
          await sendEmail(recipient.email!, payload.subject || 'Fee Message', payload.message);
        } else {
          await sendSMS(recipient.phone!, payload.message);
        }
    ...
    const finalResults = settled.map(r =>
      (r as PromiseFulfilledResult<{ recipientId: string; success: boolean; error?: string }>).value
    );
    ...
    await prisma.message.create({
      data: {
        ...
        type: payload.type,
        recipients: recipientInfo as Prisma.JsonArray,
        schoolId: tenantId,
      },
    });
  ...
  getMessages: async (
    ...
    const inboxMessages = allMessages.filter(msg => {
      const recipients = msg.recipients as any[];
