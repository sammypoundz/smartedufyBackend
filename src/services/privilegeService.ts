...
const CACHE_TTL_MS = 30_000; // 30s safety net; invalidated explicitly on change
const cache = new Map<string, CachedPrivileges>();
...
export async function getUserEffectivePrivileges(
  userId: string,
): Promise<{ roles: string[]; privileges: string[] } | null> {
 ...
 */
export function invalidatePrivilegeCache(): void {
  cache.clear();
}
