...
exports.promotionService = {
    promoteStudents: async (sourceArmId, targetArmId, studentIds, academicYearId, termId) => {
        ...
        const targetArm = await db_1.default.arm.findUnique({
        ...
        const promotedStudents = await db_1.default.student.findMany({
        ...
        await db_1.default.$transaction([
            ...promotedStudents.map(student => db_1.default.student.update({
            ...
            ...(academicYearId && termId
                ? promotedStudents.map(student => db_1.default.studentPromotionHistory.create({
                    data: {
    ...
    bulkPromoteStudents: async (flow, academicYearId, termId) => {
        const tenantId = (0, tenantContext_1.getCurrentTenantId)();
        if (!tenantId)
            throw new Error('Tenant context missing');
        if (!flow || flow.length === 0)
            throw new Error('Promotion flow is empty');
        // ---- Conflict validation ----
        ...
        for (const id of armIds) {
            if (!armMap.has(id))
                throw new Error('One or more classes do not exist or do not belong to this tenant');
        }
        ...
        await db_1.default.$transaction(moveOps.flatMap(op => [op.update, op.history]));
