...
router.post('/register', async (req, res, next) => {
    try {
        ...
        const user = await (0, tenantContext_1.runWithTenant)(schoolId, async () => {
            const idMode = data.idMode === 'MANUAL' ? 'MANUAL' : 'AUTO';
            const customId = data.customId;
            let userIdCode;
            if (idMode === 'MANUAL' && customId?.trim()) {
                userIdCode = await idGeneratorService_1.idGeneratorService.registerManualId(data.role, customId);
            }
            else {
...
router.post('/login', async (req, res, next) => {
    try {
        ...
        const rolePrivileges = await (0, roles_1.resolvePrivileges)(user.schoolId, roles);
...
router.get('/me', auth_1.authMiddleware, async (req, res, next) => {
    try {
        ...
        const privileges = (user.allowedPages || []).length > 0
            ? Array.from(new Set(user.allowedPages))
            : await (0, roles_1.resolvePrivileges)(user.schoolId, roles);
        res.json({
            id: user.id,
