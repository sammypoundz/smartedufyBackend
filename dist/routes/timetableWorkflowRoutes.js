...
const getTeacherProfile = async (req) => {
    return prisma_1.prisma.teacher.findFirst({
 ...
 */
const buildConflictReport = async (schoolId, armId) => {
    const assignments = await prisma_1.prisma.teacherSubjectAssignment.findMany({
    ...
    if (teacherIds.length) {
        const otherAssignments = await prisma_1.prisma.teacherSubjectAssignment.findMany({
        ...
        const lockedEntries = await prisma_1.prisma.timetableEntry.findMany({
            where: {
                schoolId,
        ...
        if (lockedEntries.length) {
            for (const e of lockedEntries) {
                ...
                const teacherAssignment = assignments.find((a) => a.subjectId === e.subjectId);
...
router.get("/assignments", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const where = { schoolId: req.user.schoolId };
        if (req.query.armId)
            where.armId = String(req.query.armId);
        const assignments = await prisma_1.prisma.teacherSubjectAssignment.findMany({
            where,
            include: INCLUDE,
...
router.post("/assignments", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const { subjectId, armId, teacherId, allowedDays, periodsPerWeek } = req.body;
        if (!subjectId || !armId || !teacherId)
            return res
                .status(400)
                .json({ error: "subjectId, armId and teacherId are required" });
        const days = Array.isArray(allowedDays)
            ? allowedDays.filter((d) => DAYS.includes(d))
            : [];
        ...
        const [subject, arm, teacher] = await Promise.all([
            ...
            prisma_1.prisma.arm.findFirst({
                where: { id: armId, schoolId: req.user.schoolId },
            }),
            prisma_1.prisma.teacher.findFirst({
        ...
        const assignment = await prisma_1.prisma.teacherSubjectAssignment.upsert({
            where: {
...
router.patch("/assignments/:id", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const existing = await prisma_1.prisma.teacherSubjectAssignment.findFirst({
        ...
        const updated = await prisma_1.prisma.teacherSubjectAssignment.update({
...
router.delete("/assignments/:id", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const existing = await prisma_1.prisma.teacherSubjectAssignment.findFirst({
            where: { id: String(req.params.id), schoolId: req.user.schoolId },
        });
        if (!existing)
            return res.status(404).json({ error: "Assignment not found" });
        await prisma_1.prisma.teacherSubjectAssignment.delete({
            where: { id: existing.id },
        });
...
router.get("/mine", async (req, res, next) => {
    try {
        ...
        if (!teacher)
            return res.json({ assignments: [], occupiedSlots: [] });
        const assignments = await prisma_1.prisma.teacherSubjectAssignment.findMany({
        ...
        const ownPrefs = await prisma_1.prisma.teacherSlotPreference.findMany({
            where: {
                schoolId: req.user.schoolId,
        ...
        const reviews = armIds.length
            ? await prisma_1.prisma.timetableReview.findMany({
...
router.put("/assignments/:id/slots", async (req, res, next) => {
    try {
        const { slots } = req.body;
        const assignment = await prisma_1.prisma.teacherSubjectAssignment.findFirst({
            where: { id: String(req.params.id), schoolId: req.user.schoolId },
            include: { arm: { include: { class: true } }, subject: true },
        });
        if (!assignment)
        ...
        const review = await prisma_1.prisma.timetableReview.findUnique({
        ...
        const seen = new Set();
        for (const s of list) {
            if (!s || !DAYS.includes(s.dayOfWeek) || !s.timeSlot)
                continue;
            if (assignment.allowedDays.length &&
        ...
        for (const s of clean) {
            const clash = ownPrefs.find((p) => p.dayOfWeek === s.dayOfWeek && p.timeSlot === s.timeSlot);
            if (clash) {
        ...
        const lockedArmIds = (await prisma_1.prisma.timetableReview.findMany({
            where: { schoolId: req.user.schoolId, status: "APPROVED" },
        })).map((r) => r.armId);
        if (lockedArmIds.length) {
            const myOtherAssignments = await prisma_1.prisma.teacherSubjectAssignment.findMany({
                where: {
                    schoolId: req.user.schoolId,
            ...
            if (subjectIds.length) {
                ...
                for (const s of clean) {
                    if (lockedEntries.some((e) => e.dayOfWeek === s.dayOfWeek && e.timeSlot === s.timeSlot)) {
                        return res.status(409).json({
        ...
        await prisma_1.prisma.teacherSlotPreference.deleteMany({
            where: { assignmentId: assignment.id },
        });
        if (clean.length) {
            await prisma_1.prisma.teacherSlotPreference.createMany({
                data: clean.map((s) => ({
                    schoolId: req.user.schoolId,
        ...
        const current = await prisma_1.prisma.timetableReview.findUnique({
            where: { armId: assignment.armId },
        });
        let flags = {};
        ...
        await prisma_1.prisma.timetableReview.upsert({
            where: { armId: assignment.armId },
            create: {
                schoolId: req.user.schoolId,
        ...
        const updated = await prisma_1.prisma.teacherSubjectAssignment.findUnique({
...
router.get("/arm/:armId", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const arm = await prisma_1.prisma.arm.findFirst({
        ...
        const { assignments, issues } = await buildConflictReport(req.user.schoolId, arm.id);
...
router.get("/reviews", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const reviews = await prisma_1.prisma.timetableReview.findMany({
...
router.post("/arm/:armId/request-changes", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const { note } = req.body;
        ...
        if (!arm)
            return res.status(404).json({ error: "Arm not found" });
        const review = await prisma_1.prisma.timetableReview.upsert({
            where: { armId: arm.id },
            create: {
                schoolId: req.user.schoolId,
                armId: arm.id,
...
router.post("/assignments/:id/request-changes", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const { note } = req.body;
        if (!note || !String(note).trim())
            return res.status(400).json({
        ...
        const updated = await prisma_1.prisma.timetableReview.upsert({
            where: { armId: assignment.armId },
            create: {
                schoolId: req.user.schoolId,
                armId: assignment.armId,
...
router.post("/arm/:armId/approve", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const arm = await prisma_1.prisma.arm.findFirst({
            where: { id: String(req.params.armId), schoolId: req.user.schoolId },
            include: { class: true },
        ...
        const entries = assignments.flatMap((a) => a.slots.map((s) => ({
            schoolId: req.user.schoolId,
            armId: arm.id,
            dayOfWeek: s.dayOfWeek,
            timeSlot: s.timeSlot,
        ...
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.timetableEntry.deleteMany({
                where: { armId: arm.id, schoolId: req.user.schoolId },
            }),
            ...(entries.length
                ? [prisma_1.prisma.timetableEntry.createMany({ data: entries })]
                : []),
            prisma_1.prisma.timetableReview.upsert({
                where: { armId: arm.id },
                create: {
                    schoolId: req.user.schoolId,
                    armId: arm.id,
...
router.post("/arm/:armId/unlock", (0, roleGuard_1.roleGuard)(ADMIN_ROLES), async (req, res, next) => {
    try {
        const arm = await prisma_1.prisma.arm.findFirst({
            where: { id: String(req.params.armId), schoolId: req.user.schoolId },
        });
